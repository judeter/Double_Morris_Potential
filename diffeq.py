...
#    figure( 1 )
    pl.subplot( 2, 2, 1 )
    pl.plot( t, x_euler, 'b-o', t, x_heun, 'g-o', t, x_rk2, 'r-o' )
    pl.xlabel( '$t$' )
    pl.ylabel( '$x$' )
    pl.title( 'Solutions of $dx/dt = x \sin t$, $x(0)=-1$ ($h = %4.2f$)' % h )
    pl.legend( ( 'Euler', 'Heun', '$O(h^2)$ Runge-Kutta' ), loc='lower left' )

#    figure( 2 )
    pl.subplot( 2, 2, 2 )
    pl.plot( t, x_euler - x, 'b-o', t, x_heun - x, 'g-o', t, x_rk2 - x, 'r-o' )
    pl.xlabel( '$t$' )
    pl.ylabel( '$x - x^*$' )
    pl.title( 'Errors in solutions of $dx/dt = x \sin t$, $x(0)=-1$ ($h = %4.2f$)'
           % h )
    pl.legend( ( 'Euler', 'Heun', '$O(h^2)$ Runge-Kutta' ), loc='upper left' )

#    figure( 3 )
    pl.subplot( 2, 2, 3 )
    pl.plot( t, x_rk4, 'b-o', t, x_pc4, 'g-o', t_rkf, x_rkf, 'r-o' )
    pl.xlabel( '$t$' )
    pl.ylabel( '$x$' )
    pl.title( 'Solutions of $dx/dt = x \sin t$, $x(0)=-1$ ($h = %4.2f$)' % h )
    pl.legend( ( '$O(h^4)$ Runge-Kutta', '$O(h^4)$ Predictor-Corrector', \
              'Runge-Kutta-Fehlberg' ), loc='lower left' )

#    figure( 4 )
    pl.subplot( 2, 2, 4 )
    pl.plot( t, x_rk4 - x, 'b-o', t, x_pc4 - x, 'g-o', t_rkf, x_rkf - xrkf, 'r-o' )
    pl.xlabel( '$t$' )
    pl.ylabel( '$x - x^*$' )
    pl.title( 'Errors in solutions of $dx/dt = x \sin t$, $x(0)=-1$ ($h = %4.2f$)'
           % h )
